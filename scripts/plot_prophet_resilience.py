...
def generate_comparative_outlier_plot():
    ...
    and SARIMAX (autoregressive) react differently to extreme outliers.
    """
    # Number of steps to show at the end of the timeline (Zoom)
    ZOOM_STEPS = 50 
    
    # 1. Initialize Configuration for Both Models
    with initialize(version_base=None, config_path="../configs"):
        cfg_prophet = compose(config_name="config", overrides=[
            "dataset=sp500", "corruption=outliers", "model=prophet"
        ])
        cfg_sarimax = compose(config_name="config", overrides=[
            "dataset=sp500", "corruption=outliers", "model=sarimax"
        ])
        
        # Lock seeds for reproducibility (Using Prophet config as the base for data)
        ...
        # Extract Prophet in-sample fitted values
        ...
        # --- 4. Train & Predict: SARIMAX ---
        model_s = SARIMAXModel(cfg_sarimax)
        model_s.train(train_corrupted)
        
        test_pred_s = pd.Series(model_s.predict(steps=len(test_truth)), index=test_truth.index)
        
        # Extract SARIMAX in-sample fitted values
        train_fitted_s = model_s.fitted_model.fittedvalues
        
        full_forecast_sarimax = pd.concat([train_fitted_s, test_pred_s])

        # --- 5. Subset for Visualization (Zoom) ---
        ...
        viz_forecast_sarimax = full_forecast_sarimax.iloc[-ZOOM_STEPS:]
        ...
        # D. SARIMAX Forecast
        fig.add_trace(go.Scatter(
            x=viz_forecast_sarimax.index.to_pydatetime(),
            y=viz_forecast_sarimax.values,
            mode='lines',
            name='SARIMAX',
            line=dict(color='#1F77B4', width=2.5) 
        ))
        
        # 7. Apply Architecture and Styling
        split_date = train_corrupted.index[-1].to_pydatetime()
        
        # Only draw the split line if it is within the visible window
        ...
        fig.update_layout(
            title=dict(
                text="Prophet and SARIMAX on S&P500 with Outliers at 50% Intensity",
                font=dict(size=24)
            ),
            xaxis=dict(
                title="Time",
